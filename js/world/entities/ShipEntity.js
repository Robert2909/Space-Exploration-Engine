...
export class ShipEntity {
    ...
    buildShip() {
        ...
        const bodyGeo = new THREE.CylinderGeometry(0, 2.75, 14, 12); // radiusTop = 0 hace que sea una punta perfecta
        bodyGeo.rotateX(-Math.PI / 2); // <-- Negativo para que apunte hacia -Z (el frente)
        const body = new THREE.Mesh(bodyGeo, hullMat);
        body.scale.set(1, 0.4, 1);
        body.position.set(0, 2.5, -3.5); // Desplazado hacia el frente (-Z)
        this.mesh.add(body);
        ...
        const wingGeo = new THREE.CylinderGeometry(0, 2.25, 9, 8);
        wingGeo.rotateX(Math.PI / 2);

        const wingLeft = new THREE.Mesh(wingGeo, hullMat);
        wingLeft.scale.set(1, 0.1, 1);
        wingLeft.position.set(-3.5, 2.5, 3);
        ...
        wingRight.position.set(3.5, 2.5, 3);
    ...
    alignToTerrain(generator, x, z) {
        // Altura central
        const h0 = generator.getVisualHeightAt(x, z);
        this.mesh.position.set(x, h0, z);

        // Muestrear 3 puntos pequeños alrededor para calcular la normal del terreno
        const delta = 1.0;
        const hx = generator.getVisualHeightAt(x + delta, z);
        const hz = generator.getVisualHeightAt(x, z + delta);
